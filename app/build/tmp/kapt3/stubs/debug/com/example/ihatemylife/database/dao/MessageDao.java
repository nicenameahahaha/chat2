...
public abstract interface MessageDao {
     ...
     */
    @androidx.room.Query(value = "SELECT * FROM messages WHERE source = \'telegram\' AND (senderId = :userId OR receiverId = :userId) ORDER BY timestamp ASC")
    @org.jetbrains.annotations.NotNull()
    public abstract kotlinx.coroutines.flow.Flow<java.util.List<com.example.ihatemylife.database.entities.MessageEntity>> getTelegramMessagesForUser(int userId);
